...
class Discriminator(nn.Module):
    def __init__(self, input_size, hidden_size):
        ...
        self.last_layer = nn.Sequential(nn.Linear(512, 1), nn.Sigmoid())
...
for epoch in range(200):
    for i, (imgs, _) in enumerate(dataloader):
        ...
        g_error = BCELoss(g_value, Variable(torch.ones(BATCH_SIZE) * 0.9, requires_grad=False).to(device).unsqueeze(1))
        ...
        d_real_error = BCELoss(d_real_value, Variable(torch.ones(BATCH_SIZE) * 0.9, requires_grad=False).to(device).unsqueeze(1))

        #pass fake through discriminator
        d_fake_value = discriminator(g_.detach())
        d_fake_error = BCELoss(d_fake_value, Variable(torch.zeros(BATCH_SIZE), requires_grad=False).to(device).unsqueeze(1))
